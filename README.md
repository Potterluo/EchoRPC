...
目前的RPC框架，使用Vert.x的HttpServer作为服务提供者的服务器，实现比较简单，其底层网络传输就是HTTP协议。HTTP协议中的头部信息、请求格式较重，会影响RPC的网络传输性能。
...
```
魔数:作用是安全校验，防止服务器处理了非框架发来的乱七八糟的消息(类似 HTTPS 的安全证书)版本号:保证请求和响应的一致性(类似 HTTP 协议有 1.0/2.0 等版本)
